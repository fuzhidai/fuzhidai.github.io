...
## 前置条件
- **版本信息**：v1.15.x；
- **参数信息**：采用默认参数进行加解密，即密钥类型为 aes256-gcm96；
...
### 工作流程
1. **选择散列函数**：选择一个散列函数，如 SHA-256、SHA-1 或 MD5 等；
2. **选取初始密钥**：选取一个秘密密钥作为初始密钥，该密钥只有消息的发送者和管理者才知晓；
3. **计算 HMAC**：将 “待发送的消息” 和 “初始密钥” 输入到 HMAC 算法中，算法会输出一个固定长度的哈希值 HMAC；
4. **消息发送**：将刚刚生成的 HMAC 与消息一同进行发送；
5. **认证及解密**：接收方接收到消息后，使用同样的 “密钥” 和 “散列函数” 生成一个新的 HMAC，并将这个新生成的 HMAC 与接收到的 HMAC 进行比较，如果两者一致，则认为消息在传输的过程中未被篡改，且消息确实来自于拥有密钥的发送者；
...
#### 3.1 - 加密请求处理（*backend.pathEncryptWrite）
处理的流程从上一节结束的 `*backend.pathEncryptWrite` 方法开始，首先会从请求的参数结构体 `*framework.FieldData` 中解析参数，并对参数的合法性仅仅校验。

而后，获取加解密操作的核心对象 `*keysutil.Policy`，该对象是后续所有加密操作的核心对象，对象内保存着本次加密所需的关键参数。这里需要注意的是，当我们构造获取 Policy 对象的请求参数时，会发现本次的操作类型 `*logical.Request.Operation` 为 `logical.UpdateOperation`，而非 `logical.CreateOperation`，这意味着本次加密操作，本质是一个更新操作，而非一次创建操作。

当获取到 `*keysutil.Policy` 对象后，我们就可以通过调用该对象的 `EncryptWithFactory` 方法，来执行当前密钥的加密操作。

#### 3.2 - 通过工厂进行加密（*Policy.EncryptWithFactory）
在该方法中，首先会对传入的数据进行 Base64 解码，以获取到原始的明文数据，然后再通过调用 `GetKey` 方法，来获取指定版本的密钥 `KeyEntry.Key`，该密钥将作为后续对称加密使用的密钥，在当前的场景下，即为 AES 密钥。而后通过调用 `SymmetricEncryptRaw` 方法，传入密钥版本、加密密钥、明文和对称加密参数，来对明文执行实际的对称加密操作。

执行完 `SymmetricEncryptRaw` 方法后，将会获取到对称加密后的密文，但因为最终返回的数据需要采用统一格式的编码，所以这里会再对密文进行 Base64 编码，并为编码后的数据前置增加密钥的 “版本” 信息。最终，构造响应结构体，返回加密后的密文，以及加密使用的密钥版本信息。

#### 3.3 - 同步加密原始数据（*Policy.SymmetricEncryptRaw）

概括来说，`SymmetricEncryptRaw` 方法主要执行的操作，是使用传入的加密密钥来构造 `GCM` 实例，并基于传入的对称加密参数，来生成 `GCM` 模式所需的 `nonce`，从而对传入的明文进行 `Seal` 密封操作，生成最终的密文。
